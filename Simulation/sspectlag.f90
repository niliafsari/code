...
complex, dimension(n,n) :: cspect,cspect0
...
cspect0=rspect
open(10,file='rspect.dat',access='stream')
write(10) rspect
...
do i=1,n
call four1(cspect0(1,i),n,-1)
enddo
rspect(:n/2,:)=real(cspect0(:n/2,:))
rspect(n/2+1:,:)=aimag(cspect0(:n/2,:))
call pmap('rlag.pgm',rspect,n,n,2)
open(10,file='lag.dat',access='stream')
write(10) cspect0(:n/2,:)

contains
  subroutine pmap(fn,rmap1,nx,ny,iscale1)
  real rmap(nx,ny),rmap1(nx,ny)
  integer*2, dimension(nx,ny) :: imap
  integer*1, dimension(nx,ny) :: imap1
  character(len=*):: fn
  integer npix,mypos,iscale,iscape1
